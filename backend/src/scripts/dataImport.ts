...
const achievementDefinitions = [
  { type: 'foodie_master', name: '美食达人', description: '品尝 10 种不同美食' },
  { type: 'photography_master', name: '摄影大师', description: '拍摄并上传 20 张旅行照片' },
  { type: 'exploration_pioneer', name: '探索先锋', description: '探索 10 个不同景区' },
  { type: 'social_master', name: '社交达人', description: '社交积分达到 100' },
] as const;
...
const seedAchievements = async (dataSource: DataSource, users: User[]) => {
  const achievementRepo = dataSource.getRepository(Achievement);
  const byUsername = new Map(users.map((user) => [user.username, user.id]));
  const records: Array<Partial<Achievement>> = [];

  const pushAchievements = (username: string, types: Array<(typeof achievementDefinitions)[number]['type']>) => {
    ...
    for (const type of types) {
      const definition = achievementDefinitions.find((item) => item.type === type);
      if (!definition) {
        continue;
      }
      records.push({
        ...
        earnedAt: randomPastDate(30),
      });
    }
  };

  pushAchievements('travel_admin', ['foodie_master', 'photography_master', 'social_master']);
  pushAchievements('campus_guide', ['exploration_pioneer']);
  pushAchievements('beijing_guest', ['foodie_master']);

  if (records.length > 0) {
    await insertInChunks(achievementRepo, records, 200);
  }
...
async function importData() {
  ...
  try {
    ...
    const achievementCount = await seedAchievements(dataSource, users);
    ...
    console.log(
      `补充数据：日记 ${diaryCount}（共享 ${sharedDiaries.length}），评论 ${diaryCommentCount}，用户行为 ${userBehaviorCount}，摄影打卡 ${photoCheckinCount}，社交打卡 ${socialCheckinCount}，预置成就 ${achievementCount}`,
    );
    console.log(
